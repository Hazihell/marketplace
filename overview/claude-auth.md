...
- Status monitoring for auto-refresh tokens in the macOS Keychain so you know when credentials require attention.
...
Claude Code CLI stores its authentication credentials and refresh tokens in the macOS Keychain. When authenticating headlessly or remotely, standard terminal pipes cannot complete the interactive token exchange prompt.

This plugin executes the authentication flow inside a virtual pseudo-terminal (PTY) on your machine. It extracts the OAuth authorization URL without opening a local desktop browser, presents it through the BB interface or agent chat, feeds your authorization code back to the CLI, and inspects Keychain status to verify that the session is active.
...
The plugin includes a clean command and button that signs out of Claude Code by deleting its Keychain credentials and clears dead OAuth flags from configuration files, allowing a fresh sign-in when sessions get stuck.
